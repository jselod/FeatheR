...
##Introduction:
The goal of this project is to use eBird and meteorological data to study the effect of local weather conditions on the arrival time of trans-Gulf migrant (TGM) species in Massachusetts. Specifically, we are examining if arrival time of five different species varies according to temperature and wind variables along migration route for these TGMs migrating to Massachusetts. Once considering that arrival time may vary with meteorological conditions, we will consider what role climate change will potentially play in the population status of TGMs arriving in MA during the spring. 

##Methods:
We began by downloading eBird’s species occurrence data from the Global Biodiversity Information Facility (GBIF) and selecting 5 TGMs to analysze. We then compiled weather using rnoa which contains functions that interact with NOAA’s National Climatic Data Center’s API.
...
##Author Contributions: The group met outside of class to split the work up. Jaan inserted the necessary code chunks. Elisabeth created the bibliography and crafted the Introduction and Author Contributions. 
##References:
