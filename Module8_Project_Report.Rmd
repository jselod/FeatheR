...
mc.lmer <- lmer(j.day~tmin*tmax*wvec+(1|name),po.arr.weath,na.action = "na.fail")
Anova(mc.lmer)

#0Mean two week weather preceding arrival
mc.lmer2 <- lmer(j.day~wk.tmin*wk.tmax*wk.wvec+(1|name),po.arr.weath2,na.action = "na.fail")
Anova(mc.lmer2)

po.arr.aic <- dredge(mc.lmer2,fixed = c("wk.tmin","wk.tmax","wk.wvec"),)

mc.kb <- kable(po.arr.aic[1:4,],caption = "Fit values for nested models of the most complicated lme model")
